# Author: Hongyan Wang
# Data: June 15, 2014
# Original: http://www.codeskulptor.org/#user38_At8gBZYhDC_1.py
...
class ImageInfo:
    def __init__(self, center, size, radius = 0, lifespan = None, animated = False):
    	"""
    	Inputs:
    	       center is a list of length 2
    	       size is a list of length 2
    	       radius is an integer
    	       lifespan is an integer
    	       animated is boolean type
    	"""
        self.center = center
        self.size = size
        self.radius = radius
    ...
    def get_center(self):
    	"""
    	Inputs: ImageInfo object
    	Outputs: the center of the image
    	"""
        return self.center
    ...
    def get_radius(self):
    	"""
    	Inputs: ImageInfo object
    	Outputs: the radius of the image
    	"""
        return self.radius
    ...
    def get_lifespan(self):
    	"""
    	Inputs: ImageInfo object
    	Outputs: the lifespan of the image
    	"""
    ...
    def get_animated(self):
    	"""
    	Inputs: ImageInfo object
    	Outputs: whether image is animated
    	"""
...
def group_collide(group,other_object):
    ...
    for item in group:
        if item.collide(other_object):
            remove_set.add(item)
            a_explosion = Sprite(item.get_pos(),[0,0],0,0,explosion_image,explosion_info,explosion_sound)
...
def draw(canvas):
    global time, started,lives,missile_group,score,rock_group
...
def rock_spawner():
    ...
    if dist(rock_pos,my_ship.get_pos()) > (asteroid_info.get_radius()+my_ship.get_radius()):
        ...
        if started and (len(rock_group) < 13):
